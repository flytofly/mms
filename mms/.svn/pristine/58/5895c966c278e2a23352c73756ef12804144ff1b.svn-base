	...
	public Page<Task> paginate(int pageNumber, int pageSize) {
		...
		String sql =" from cy_business.cy_business_project_detail t"
			...
			+" (select ccr.detail_id,group_concat(cro.cname separator '--') cnames "
				...
				+" where  t.project_type = 6 and t.status!=9 and o.detail_id is not null ";
...
/*		String sql = "select t.*,o.out_type,b.mid,b.name,b.content,cb.business_name from cy_business.cy_business_project_detail t,"
   ...
   return Db.queryInt(sql);
	...
	//首页任务统计list
	public List<Record> TaskStatistics(){
		String subsql="";
		if(!StringUtil.isNull(this.getStr("out_type"))){
			subsql+=" and b.out_type="+this.getStr("out_type");
		}if(!StringUtil.isNull(this.getStr("begindate"))&&!StringUtil.isNull(this.getStr("enddate"))){
			subsql+=" and a.begindate= '"+this.getStr("begindate")+"  00:00:00 'and a.endtate='"+this.getStr("enddate")+"00:00:00 '";
		}
		
		
		String sql="select aa.detail_id,aa.detail_name,ifnull(aa.canpick_day,0) canpick_day, "
				+ "ifnull(aa.minreqnum_day,0) minreqnum_day,ifnull(sum(bb.picknumber),0) picknumber, "
				+ "ifnull(sum(bb.fb_count),0) fb_count,ifnull(sum(bb.yx_count),0) yx_count,"
				+ "ifnull(sum(bb.yxa_count),0) yxa_count,"
				+ "ifnull(sum(bb.yxb_count),0) yxb_count, ifnull(sum(bb.yx_count)/sum(bb.fb_count),0) zhl,"
				+ "aa.out_type"
				+ " from (SELECT a.detail_id,a.detail_name,a.canpick_day,a.minreqnum_day,b.out_type FROM cy_business.cy_business_project_detail a,cy_business.cy_business_project_detail_output b"
				+ "  WHERE a.project_type = 6 and  a.detail_id = b.detail_id  and a.`status` != 9 and a.`status`=1 and a.use_status=1"+subsql
				+ ") aa "
