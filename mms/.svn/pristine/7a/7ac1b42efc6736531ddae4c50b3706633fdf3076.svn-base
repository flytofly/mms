...
public class CrowdController extends Controller {
	...
	public void list() {
		...
		if (StrKit.notBlank(submit)) {
			...
			setAttr("records", record.paginate(1, 5));
		} else {
			setAttr("records", record.paginate(pageNumber, 5));
		}
	...
	public void edit() {
		...
		render("/WEB-INF/jsp/data/crowd/editcrowd.html");
	}
	
    public void editrulecrowd(){
    	render("/WEB-INF/jsp/data/crowd/editrulecrowd.html");
    }
	...
	public void getLevelSelect() {
		List<Record> province = Crowd.dao.findAreaList();
	HashMap<String , String> map=	new HashMap<String , String>();
	
		for (Record record : province) {
			String id=	record.getStr("pid");
	...
	private void processScenes(String sids, int cid, Crowd record) {
     ...
     // 
	private void processAreas(int cid) {
         
		
	    CrowdArea crowdArea=new CrowdArea();
	    crowdArea.deleteAreasByCid(cid);
		...
		if (!StrKit.isBlank(areas)) {
			...
			for (int i = 0; i < provinceIdArr.length; i++) {
				...
				String tmp = provinceIdArr[i];
				if (tmp != null && tmp.length() > 1 && !"0".equals(tmp)) {
					
					List <Record>list=crowdArea.findArea(tmp);
					if(list.size()>0){
						area.put("cid", cid);
						area.put("province_id",list.get(0).get("provinceID"));
						area.put("province",list.get(0).get("province"));
						area.put("city_id", list.get(0).get("cityID"));
						area.put("city",list.get(0).get("city"));
						areaList.add(area);
					}
...
/*	private void processArea(int cid) {
	...
	public void area(){
		...
		render("/WEB-INF/jsp/data/crowd/area.html");
	}
