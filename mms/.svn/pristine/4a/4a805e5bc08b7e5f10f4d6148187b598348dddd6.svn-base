...
public class AppSeqPos
{
	...
	private HashMap<String,DomainUnit> _domain_map;
	...
	*/
	...
	{
		if (_domain_map == null) {
			_domain_map = new HashMap<String,DomainUnit>();
		}
	...
	*/
	private DomainUnit[] getAllDomainUnits()
	{
		...
		Iterator<Map.Entry<String,DomainUnit>> iter = _domain_map.entrySet().iterator();
		while (iter.hasNext()) {
			Map.Entry<String,DomainUnit> entry = iter.next();
			DomainUnit node = entry.getValue();
